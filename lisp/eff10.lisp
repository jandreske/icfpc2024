(defvar biggest 21847450052839212624230656502990235142567050104912751880812823948662932355201)
...
(defun make-fixed-indices ()
  (let ((table (make-array 81 :element-type '(integer -1 8) :initial-element -1)))
    (dolist (cl (cdr sat-expr))
      (when (eq (car cl) '=)
	(setf (aref table (var-to-index (cadr cl))) (- (caddr cl) 1))))
    table))
...
(defparameter excl (make-excl-table))
(defparameter fixed (make-fixed-indices))

(declaim (ftype (function ((simple-array bit (81 81))
			   (simple-array (integer -1 8) (81)))
			  list)
		build-eff10))	
(defun build-eff10 (exclusions fixed)
  (let ((digits (make-array 81 :element-type '(integer 0 8))))
    (labels ((try-fill (i)
	       (block try-fill
		 (cond ((> i 80) t)
		       ((>= (aref fixed i) 0)
			(when (can-fill i (aref fixed i) digits exclusions)
			  (setf (aref digits i) (aref fixed i))
			  (let ((r (try-fill (+ i 1))))
			    (when r
