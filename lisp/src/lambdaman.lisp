...
(defun adjacent-pills-count (grid x y)
  (flet ((safe-pill-on (x y)
	   (if (and (>= x 0) (< x (grid-width grid))
		    (>= y 0) (< y (grid-height grid))
		    (char= (grid-ref grid x y) #\.))
	       1 0)))
    (+ (safe-pill-on (- x 1) y)
       (safe-pill-on (+ x 1) y)
       (safe-pill-on x (- y 1))
...
(defun find-nearest (grid pos c)
  ...
  (let ((min-dist 1000000)
	(best-pos nil)
	(adjacent-pills 0)) ; hack for visiting dead ends first
	...
	(when (char= (grid-ref grid x y) c)
	  (let ((dist (manhattan-distance-xy pos x y)))
	    (when (or (< dist min-dist)
		      (and (= dist min-dist)
			   (< (adjacent-pills-count grid x y) adjacent-pills)))
	      (setf min-dist dist)
	      (setf adjacent-pills (adjacent-pills-count grid x y))
